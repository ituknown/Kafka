...
因为每个消费者组独立消费，消费者组之间互相不影响。

比如如下几个服务同时订阅一个 topic，但是每个服务的重要程度不一，而且实例数也不相同：
...
# 2. 副本数（replication-factor）至少设为 3
...
# 3. 设置 min.insync.replicas 确保消息可靠性
...
# 4. cleanup.policy：delete/compact 按用途选择
...
# 5. retention.ms / retention.bytes 要合理
